...
class DetailsAsin:
    ...
    def get_data(self):
        ...
        try:
            extra_details = {}
            bsr = [i.strip() for i in soup.select_one('#detailBullets_feature_div').text.splitlines() if i]
            if 'Product details' in bsr[0]:
                bsr = [i.strip() for i in soup.select_one('#detailBullets_feature_div').text.splitlines() if i][1:]
            print(bsr)
